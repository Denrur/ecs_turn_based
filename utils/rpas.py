...
def _combine_obstructions(old, new):
...
def _cell_at(x_center, y_center, quad_x, quad_y,
    ...
    def _visible_cells_in_octant_from(self, x_center, y_center,
        ...
        while iteration <= radius and not (len(obstructions) == 1 and
            ...
            for step in range(iteration + 1):
                cell = _cell_at(x_center, y_center,
                                quad_x, quad_y,
                                step, iteration, is_vertical)

                if self._cell_in_radius(x_center, y_center, cell, radius):
                    cell_angles = CellAngles(near=(step * angle_allocation),
...
if __name__ == '__main__':
    ...
    map_ = [[(lambda: prob_is_obstructed < random.uniform(
    ...
    def is_unobstructed(x, y):
        try:
            ...
            return False
    fov = FOVCalc()
    print(sorted(fov.calc_visible_cells_from(center_x, center_y, radius, is_unobstructed)))
